...
from lyra_geometry import TensorSpace, geodesic_equations


def test_geodesic_equations_minkowski_timelike():
    t, x, y, z = sp.symbols("t x y z")
    metric = sp.diag(1, -1, -1, -1)
    space = TensorSpace(coords=(t, x, y, z), metric=metric)
    equations = space.geodesic_equations(parameter="timelike")
...
def test_geodesic_equations_minkowski_null_lambda():
    t, x, y, z = sp.symbols("t x y z")
    metric = sp.diag(1, -1, -1, -1)
    equations = geodesic_equations(metric, (t, x, y, z), parameter="null")
    assert len(equations) == 4
